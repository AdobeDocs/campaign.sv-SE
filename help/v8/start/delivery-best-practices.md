...
### Leverans och underhåll {#delivery-maintenance}

Regelbundet underhåll av leveransen är nödvändigt för optimala prestanda på plattformen.

+++ **Läs om några metodtips**

* **Ta bort misslyckade och onödiga leveranser**: Behåll inte leveranser i feltillstånd för instansen eftersom detta bevarar temporära tabeller och påverkar prestanda. Ta regelbundet bort leveranser som inte längre behövs för att frigöra systemresurser.

* **Rensa inaktiva mottagare**: Inaktiva mottagare under de senaste 12 månaderna bör tas bort från databasen för att upprätthålla adresskvaliteten. Internetleverantörer inaktiverar adresser efter en tids inaktivitet och studsade meddelanden skickas till avsändare för att informera dem om den nya statusen. Regelbunden rensning av listor förbättrar slutresultatet och minskar kostnaderna.

* **Schemalägg stora leveranser klokt**: Schemalägg inte stora leveranser tillsammans. Samordna schemaläggningen av leveranser med övriga medlemmar i teamet för att sprida belastningen enhetligt över systemet. När flera stora leveranser skickas samtidigt kan detta påverka plattformens totala prestanda.
...
Vissa e-postklienter blockerar bilder som standard, och användare kan ändra sina inställningar till att blockera bilder för att spara på dataanvändningen.  Om bilderna inte hämtas kan därför hela meddelandet gå förlorat.
...
* Från leveransassistenten kan du importera en HTML-sida som innehåller bilder eller infoga bilder direkt med HTML-redigeraren via ikonen **[!UICONTROL Image]**.
...
[Läs mer om spårningsfunktioner](../send/tracking.md)
