...
>[!CAUTION]
>
>Observera att omdirigerings-URL alltid ska ha `oauth.jsp` som mål på din Campaign-programserverdator via HTTPS (port 443). Serverdomäner med understreck stöds inte heller när OAuth används. Använd serverdomäner utan understreck där avsikten är att använda OAuth.
